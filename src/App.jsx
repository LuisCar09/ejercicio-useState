...
// 1. Clonate este repositorio y en el componente funcional `App`, importa `useState` desde React. DONE
...
function App() {
    ...
    const ChangeName = (event) => {
        //setNewName(event.target.textContent)
    }
    // useEffect(()=> {
    //     newName ? setName(newName) : null
    //     setNewName('')
    ...
    return (
        <div>
            ...
            <ul>
                <li onClick={(event)=> { ChangeName(event)}}>Data</li>
                <li onClick={(event)=> { ChangeName(event)}}>Reyes</li>
                <li onClick={(event)=> { ChangeName(event)}}>Yolanda</li>
            </ul>

            <form onSubmit={(event) => inputChangeName(event)} >
                <input type='text' id='name' value={newName} onChange={(event)=> ChangeNewName(event)} placeholder='Add a name' />
